...
参考文章：[http://www.ruanyifeng.com/blog/2016/04/cors.html]() ， [https://segmentfault.com/a/1190000015597029#articleHeader0]()

### **一、何为跨域**

#### 1.  浏览器的同源策略Same-origin Policy

      如果两个url的协议、域名或ip、端口都相同，即在浏览器中被视为两者同源。下表给出了相对`http://store.company.com/dir/page.html`同源检测的示例:
...
      CORS是一个W3C标准，全称是"跨域资源共享"（Cross-origin resource sharing）。它允许浏览器向跨源服务器，发出[`XMLHttpRequest`]()请求，从而克服了AJAX只能[同源]()使用的限制。也就是说CORS是为了解决跨源访问问题的一套机制。
...
      设有两个服务器A和B，A的页面调用A的接口，没有问题；但是A的页面调用B的接口，将会产生跨域问题。

进入A的index页面

![](https://oscimg.oschina.net/oscnet/499cc41e9a1a47946f4bb2f5eae84584015.jpg)
...
![](https://oscimg.oschina.net/oscnet/97ca66325a7dade2c730d779de979e12a45.jpg)

![](https://oscimg.oschina.net/oscnet/0bb6b4580d642fd9e7fb506553e3d5e8727.jpg)

访问正常，但是当点击右边按钮时，就会报错

![](https://oscimg.oschina.net/oscnet/282bb2fb76a409602bec08b207bd6e818d9.jpg)
...
![](https://oscimg.oschina.net/oscnet/7b81763d418e2cd5d48fb559369244dc161.jpg)
...
整个CORS通信过程，都是浏览器自动完成，不需要用户参与。对于开发者来说，CORS通信与同源的AJAX通信没有差别，代码完全一样。浏览器一旦发现AJAX请求跨源，就会自动添加一些附加的头信息，有时还会多出一次附加的请求，但用户不会有感觉。如果需要传递cookie信息，则需要前端额外设置一些信息。
...
该字段是必须的。它的值要么是请求时`Origin`字段的值，要么是一个`*`，表示接受任意域名的请求, 但是如果要传递cookie，那么就不能设置*。
...
Spring Framework还提供了[CorsFilter]()。在这种情况下，可以在Spring Boot应用程序中声明过滤器：
